...
$statusLabel.Text = "Please wait until finished ..."
    ...
    $RF2ROOT = $shell.BrowseForFolder( 0, 'Select a folder to install', 16, $shell.NameSpace( 17 ).Self.Path ).Self.Path

    if ( "$RF2ROOT" )
        {

    if ("$RF2ROOT" -eq "") { . "$CURRENTLOCATION\variables.ps1" }
    ...
    start-process $STEAMINSTALLDIR\steamcmd "+force_install_dir $RF2ROOT +login anonymous +app_update 400300 -beta $RF2BETAVERSION +quit 1> _output.txt 2>&1" -nonewwindow -wait|out-null
        
    (get-content "$CURRENTLOCATION\variables.ps1") -replace "RF2ROOT=.*","RF2ROOT=""$RF2ROOT""" | set-content -Path "$CURRENTLOCATION\variables.ps1"
    #Add-Content -Path $pwd\variables.ps1 -Value -Value "`$RF2ROOT=""$RF2ROOT""" -Encoding ASCII
...
& $CURRENTLOCATION\helper\rfcmp_extractor.ps1|out-null
...
# New-Item -Path $RF2ROOT\Packages -Name "dummy.dat" -ItemType "file" -Value "$PKGINFO"
...
{
cp $CURRENTLOCATION\modfiles\$FILE $RF2ROOT\Packages
}

#
# create MAS file
$textBox.AppendText("`r`nCreating mas-file for initial mod.")
#& "$RF2ROOT\bin64\modmgr.exe" -c""$RF2ROOT"" -o""$RF2ROOT\Packages"" -m""dummy.mas"" ""$RF2ROOT\Packages\default.rfm"" ""$RF2ROOT\Packages\icon.dds"" ""$RF2ROOT\Packages\smicon.dds""
...
#start-process -FilePath "$RF2ROOT\bin64\rFactor2 Dedicated.exe" -ArgumentList $ARGUMENTS -NoNewWindow -Wait
